...
function setup() {
    ...
    for (let i = 0; i < virus_count; i++) {
        ...
        if (abs(width / 2 - virus[i][0]) < virus_distance) {
            virus[i][0] = virus[i][0] - (width / 2 - virus[i][0]);
        }
        if (abs(height / 2 - virus[i][1]) < virus_distance) {
            virus[i][1] = virus[i][1] - (height / 2 - virus[i][1]);
        }
    }
...
function draw() {
    ...
    switch (mode) {
        ...
        case 0:
            ...
            fill(random(255), random(255), random(255), 50);
            text('use CHROME or BRAVE to HAVE FUN!', width / 2, height / 2 + 7 * textSIZE);
        ...
        case 1:
            ...
            for (let i = 0; i < virus_count; i++) {
                if (virus[i][2] === 1) {
                    ...
                    if (contact(virus[i][0], virus[i][1], player[0][0], player[0][1])) {
                        dead = true;
                        mode = 2;
                    }
    ...
    for (let i = 0; i < projectile_count; i++) {
        if (projectile[i][4] != 0 && millis() - projectile[i][4] < projectile_life) //projectile is alive!
        {
            ...
            for (let k = 0; k < virus_count; k++) {
                if (virus[k][2] === 1) {
                    if (contact(virus[k][0], virus[k][1], projectile[i][0], projectile[i][1])) {
                        virus[k][2] = 0;
                        projectile[i][4] = 0;
                        frag_left--;
                    }
